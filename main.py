...
Usage:
    python main.py --weights weights/yolov5s_face.onnx --source assets/bus.jpg
    python main.py --weights weights/yolov5m_face.onnx --source 0  # webcam
    python main.py --weights weights/yolov5s_face.onnx --source video.mp4
"""
...
from utils.general import LoadMedia, check_img_size, increment_path, scale_boxes, scale_landmarks
...
def draw_face_detections(image: np.ndarray, box: np.ndarray, score: float, landmarks: np.ndarray) -> None:
    ...
    # Put text on the image (no background)
    cv2.putText(
        image,
        label,
        (x1, y1 - 5),
...
def run_face_detection(
    ...
    for resized_image, original_image, status in dataset:
        ...
        if len(boxes) > 0:
            ...
            landmarks = scale_landmarks(resized_image.shape, landmarks, original_image.shape)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--weights",
        type=str,
        default="weights/yolov5m_face.onnx",
